...
public class GameState_2 implements IState {
    // 여기가 2탄
    ...
    int player_missile_regen_speed =800;//0.5초마다 미사일 나감
    int bomb=2;// 궁극기 개수
    ...
    ArrayList<RandomBox> m_randomboxList = new ArrayList<RandomBox>();// 랜덤박스 변수
    ...
    public GameState_2()// Appmanager에 GameState_2등록
    {

        AppManeger.getInstance().m_gamestate_2 = this;
    }
    ...
    public void CheckCollision()
    {
        ...
        if(boss!=null)
        {
            if(CollisionManager.CheckBoxToBox(boss.m_BoundBox,m_player.m_BoundBox))
            {
        ...
        for (int i = m_randomboxList.size() - 1; i >= 0; i--) {
            if (CollisionManager.CheckBoxToBox(m_player.m_BoundBox, m_randomboxList.get(i).m_BoundBox)) {
                ...
                else if (m_randomboxList.get(i).BoxType == 3)//  궁극기 수 증가
                {
                    if(subcnt<4)
                        subcnt++;
        ...
        if(boss!=null)
        {
            if(AppManeger.getInstance().subMissilecnt>=10)
            {
    ...
    public void MakeEnemy()// 적을 생성
    {
        ...
        {
            if (System.currentTimeMillis() - LastRegenEnemy >= 1500) // 1초가 지나면 적들이 생성
            {
                LastRegenEnemy = System.currentTimeMillis();

                Enemy enem = null;
                int enemtype = randomEnem.nextInt(4);
                ...
                enem.movetype = randomEnem.nextInt(4);// 랜덤으로 무브타입 생성
                enem.speed=10+randomEnem.nextInt(15);// 잡몹들 스피드
    ...
    public void MakeRandomBox()// 랜덤 박스 생성
    {
        if(boss==null)
        {
            if (System.currentTimeMillis() - LastRegenBox >= 3000) {
                LastRegenBox = System.currentTimeMillis();

                RandomBox m_randomBox = null;
                int boxType = randomBox.nextInt(4);
                ...
                if (boxType == 0)
                    ...
                    m_randomBox = new RandomBox_add_bomb();// 궁극기 수 증가
    ...
    public void MakePlayerMissile()// 미사일을 생성
    {
        ...
        if(System.currentTimeMillis()-PlayerMissile>=1000) {
            ...
            {
                ;// 여기서 플레이어와 같은 위치에서 미사일이 나감
                Missile_sub1 sub1 = new Missile_sub1(x-150, y-100, player_missile_speed);// 미사일 위치,위치,속도
    ...
    public void Update()
    {
        ...
        if(boss!=null)
        {
            ...
            if(boss.state==Boss.STATE_OUT) {
                ...
                AppManeger.getInstance().getGameView().changeGameState(new GameState_3());
            }
        }
    ...
    public void Render(Canvas canvas) {// 여기서 업데이트된 이미지들을 그려준다
        ...
        canvas.drawText(" 2 Stage",width/10*4,100,p);// 스테이지 이름
