...
import kr.co.apps.gameactivity_framework.GameViewThread;
...
public class GameState_1 implements IState {
    ...
    public void MakeMS()
    {//궁극기 토마호크의 객체를 생성하고 위치를 지정하고 어레이 리스트에 넣음
    ...
    ArrayList<Missile_Tomahawk> t_tom = new ArrayList<Missile_Tomahawk>();//토마호크 미사일 리스트
    Tomahawk chk1,chk2,chk3;

    int player_hp=10;
    int player_missile_speed=70;
    int player_missile_regen_speed=1000;
    int bomb=2;
    ...
    ArrayList<Missile_Enemy> m_enemmslist = new ArrayList<Missile_Enemy>();// 적들 미사일 변수
    ...
    public GameState_1()// Appmanager에 GameState_1등록
    {
        AppManeger.getInstance().m_gamestate_1 = this;
    }
    //충돌 확인 메서드
       ...
       for(int i=t_tom.size()-1;i>=0;i--)
        ...
        if(subcnt>=3) {
            for (int i = sub1List.size() - 1; i >=0 ; i--) {
                for (int j = m_enemlist.size() - 1; j>= 0; j--) {
                    if (CollisionManager.CheckBoxToBox(sub1List.get(i).m_BoundBox, m_enemlist.get(j).m_BoundBox)) {
                        sub1List.get(i).state = Missile.STATE_OUT;
                        //sub1List.remove(i);
                        m_enemlist.get(j).state = Missile.STATE_OUT;
                        m_enemlist.remove(j);
            ...
            for(int i=sub2List.size()-1;i>=0;i--)
            {
                for(int j=m_enemlist.size()-1;j>=0;j--)
                {
                    if(CollisionManager.CheckBoxToBox(sub2List.get(i).m_BoundBox,m_enemlist.get(j).m_BoundBox))
                    {
        ...
        // 보조 플레이어1,2 + 보스
        if(subcnt>=3 && boss!=null)
        {
            for(int i=sub1List.size()-1;i>=0;i--)
            {
                if(CollisionManager.CheckBoxToBox(boss.m_BoundBox,sub1List.get(i).m_BoundBox))
                {
                    boss.hp--;
                    sub1List.get(i).state=Missile.STATE_OUT;
                    sub1List.remove(i);
            ...
            for(int i=sub2List.size()-1;i>=0;i--)
            {
                if(CollisionManager.CheckBoxToBox(boss.m_BoundBox,sub2List.get(i).m_BoundBox))
                {
                    boss.hp--;
                    sub2List.get(i).state=Missile.STATE_OUT;
                    sub2List.remove(i);
    ...
    public void MakeEnemy()// 적을 생성
    {
        if(m_background.getY()<-1000)
            if (System.currentTimeMillis() - LastRegenEnemy >= 1000) // 3초가 지나면 적들이 생성
            {
                ...
                Enemy enem = null;
                int enemtype = randomEnem.nextInt(3);
                ...
                enem.movetype = randomEnem.nextInt(3);// 랜덤으로 무브타입 생성
                enem.speed=10+randomEnem.nextInt(10);
                m_enemlist.add(enem);
            }
    }
    ...
    public void MakeRandomBox()// 랜덤 박스 생성
    {
        if(boss==null) {
            if (System.currentTimeMillis() - LastRegenBox >= 2500) {
                ...
                int boxType = randomBox.nextInt(3);
    ...
    public void MakePlayerMissile()// 미사일을 생성
    {
        ...
        if(System.currentTimeMillis()-PlayerMissile>=1000) {
            ...
            ;// 여기서 플레이어와 같은 위치에서 미사일이 나감
            Missile_Player pms = new Missile_Player(x-40, y-150, player_missile_speed);// 미사일 위치,위치,속도
            ...
            {
                ;// 여기서 플레이어와 같은 위치에서 미사일이 나감
                Missile_sub1 sub1 = new Missile_sub1(x-190, y-100, player_missile_speed);// 미사일 위치,위치,속도
                Missile_sub2 sub2=new Missile_sub2(x+60,y-100,player_missile_speed);
    ...
    @Override
    public void Update()// 업데이트 후-> render로 그린다
    {
        ...
        if(boss!=null)
        {
            ...
            if(boss.state==Boss.STATE_OUT) {
                boss = null;
                //보스가 죽으면 다음 스테이지로 넘어감
                AppManeger.getInstance().m_gamestate_1 = this;

                AppManeger.getInstance().getGameView().changeGameState(new GameState_2());
            }
        }
        ...
        if(System.currentTimeMillis() - TWTime >= 5000 && twflag == 1) {//5초뒤 제거
            twflag = 0;
            c_chk.clear();
            t_tom.clear();
        }
    ...
    public void Render(Canvas canvas) {// 여기서 업데이트된 이미지들을 그려준다
        ...
        canvas.drawText(" 1 Stage",width/10*4,100,p);// 스테이지 이름
        ...
        if(boss!=null) {
            boss.Draw(canvas);
            canvas.drawText("보스 목숨" + boss.hp,10,100,p);
        }
        ...
        //적 미사일 내려가는거 그려줌
        for (Missile_Enemy enemms : m_enemmslist)
            enemms.Draw(canvas);
